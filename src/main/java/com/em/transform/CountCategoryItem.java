...
public class CountCategoryItem extends RichFlatMapFunction<Event, Tuple2<Long, Integer>> {
    ...
    @Override
    public void flatMap(Event event, Collector<Tuple2<Long, Integer>> out) throws Exception {
        ...
        for (Rule rule : rules) {
            if (rule.getCategory().equals(category) && rule.getItemId() == itemId) {
